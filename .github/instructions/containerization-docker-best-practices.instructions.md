...
This instructions file contains detailed containerization and Docker best practices for maintainers and Copilot guidance.
...
- Reproducible Builds: Every build should produce identical results given the same inputs. Pin dependency versions and control build environments.

### 2. Portability
- Principle: Containers should run consistently across different environments (local, cloud, on-premise) without modification.
- Externalize configuration via env vars and avoid hardcoding environment specifics.

### 3. Isolation
- Principle: Containers provide process and resource isolation to prevent interference.
- Use CPU/memory limits and container networks for inter-container communication.

## Dockerfile Best Practices

### 1. Multi-Stage Builds
- Use multiple `FROM` instructions to separate build-time dependencies from runtime dependencies; copy only necessary artifacts into the runtime stage.

### 2. Choose the Right Base Image
- Prefer minimal and official images (alpine, slim) and pin versions.

### 3. Optimize Image Layers
- Combine `RUN` commands, clean up package lists in the same `RUN`, and order instructions for cache efficiency.
...
- Create a non-root user and use `USER` to run processes; use `EXPOSE` to document the listening port.

### 7. Use `CMD` and `ENTRYPOINT` Correctly
- Prefer the exec form for signal handling: `CMD ["executable","arg"]`.
...
- Configure resource limits, logging & monitoring, persistent storage, and networking best practices for orchestration platforms like Kubernetes.
